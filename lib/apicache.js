  ...
  this.middleware = function cache(duration, middlewareToggle) {
    ...
    return function cache(req, res, next) {
      ...
      cacheSystem.get(req.url, function(err, reply) {
        ...
        if (cached = reply) {
          if (globalOptions.debug) {
            console.log('[api-cache]: returning cached version of "' +
             req.url + '"');
          }
          ...
          if (globalOptions.debug) {
            console.log('[api-cache]: path "' + req.url +
              '" not found in cache');
          }
            ...
            if (bypass2) {
              if (globalOptions.debug) {
                if (req.apicacheGroup) {
                  console.log('[api-cache]: group detected: ' +
                    req.apicacheGroup);
                ...
                console.log('[api-cache]: adding cache entry for "' + req.url +
