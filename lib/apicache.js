...
function cacheDriver() {
  ...
  this.delGroup = function(group, callback) {
    log.debug('deleting group %s', group);
    group = prefixKey('group:' + group);
    client.smembers(group, function(err, results) {
      async.each(results, this.del, function(err) {
        if (err) {
          log.error(err);
          callback(err);
          return;
  ...
  this.get = function(key, callback) {
    ...
    client.get(key, function(err, reply) {
      log.debug(key + ': took %d to read from redis', (new Date().getTime() - start));
      start = new Date().getTime();
      var response = JSON.parse(reply);
      log.debug(key + ': took %d to parse response', (new Date().getTime() - start));
  ...
  this.set = function(key, response, duration) {
    ...
    log.debug(key + ': took %d to stringify response', (new Date().getTime() - start));
    start = new Date().getTime();
    return client.setex(
        key,
        ...
        json,
        function() {
          log.debug(key + ': took %d to write to redis', (new Date().getTime() - start));
        }
    );
  };
...
function ApiCache() {
  ...
  var log = bunyan.createLogger({
    name: 'apicache',
    level: logLevel,
    streams: [ { level: logLevel, type: 'raw', stream: prettyStdOut } ]
  });

  var index = null;
  var cacheSystem = cacheDriver();
  cacheSystem.setDriver(globalOptions.driver);
  cacheSystem.setLogger(log);
